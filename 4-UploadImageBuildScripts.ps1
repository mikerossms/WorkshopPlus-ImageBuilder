<#
.SYNOPSIS
    Uploads image build scripts to a specified storage account.

.DESCRIPTION
    This script uploads image build scripts to a specified storage account. It reads configuration settings from a 
    JSON file, optionally performs a login, and uploads the scripts to the designated container in the storage account.
...
.PARAMETER buildScriptsFolder
    The local path to the folder containing the image build scripts. Default is "./ImageBuildScripts".

.EXAMPLE
    .\4-UploadImageBuildScripts.ps1 -configFilePath "./config.json" -dologin $true buildScriptsFolder "./ImageBuildScripts"
